...
```{r cpm}
counts_per_million <- cpm(x)
plotDensities(counts_per_million, group=group)
log2cpm <- cpm(x, log = TRUE)
plotDensities(log2cpm, group = group)
```

Plot density of expression values for each sample.

```{r plotDensities}
plotDensities(counts_per_million, group = group, main = "Raw")
```
...
```{r}
plot(log2cpm["YML123C", ], las = 2, cex.names = 0.75)
```
